...
RUN --mount=type=cache,id=cache-apk,sharing=locked,target=/var/cache/apk \
    apk add --update git
COPY rebar.config rebar.lock .
RUN --mount=type=cache,id=cache-rebar,target=/root/.cache/rebar3 \
    rebar3 compile


FROM builder as prod_compiled
RUN --mount=type=bind,target=. \
    --mount=type=cache,id=cache-rebar,target=/root/.cache/rebar3 \
